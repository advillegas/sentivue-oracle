...
#   <root>/state/roo-import.json  Roo Code provider profile (auto-imported on
#                                 startup via roo-cline.autoImportSettingsPath)
...
ctx_anchor="$(ctx_of "$anchor")"
[[ "$ctx_anchor" =~ ^[0-9]+$ ]] || ctx_anchor=32768
mkdir -p "$ROOT/state"
cat > "$ROOT/state/roo-import.json" <<EOF
{
  "providerProfiles": {
