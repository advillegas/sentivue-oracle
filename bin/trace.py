...
def main() -> int:
    ...
    p = sub.add_parser("list"); p.add_argument("--mission", default=None)
    p = sub.add_parser("show"); p.add_argument("log")
    p = sub.add_parser("diff"); p.add_argument("a"); p.add_argument("b")
    p = sub.add_parser("grep"); p.add_argument("pattern"); p.add_argument("--mission", default=None)
    args = ap.parse_args()
    if args.cmd == "list":
