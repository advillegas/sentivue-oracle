    ...
    "desk" {
        ...
        if (-not (Test-Path $bin)) {
            if (-not (Get-Command cargo -ErrorAction SilentlyContinue)) {
                Write-Host "==> installing rust (one time, winget)"; winget install --id Rustlang.Rustup -e --silent
                $env:PATH = "$env:USERPROFILE\.cargo\bin;" + $env:PATH
            }
            Write-Host "==> building oracle-desk (one time)"
            Push-Location (Join-Path $Root "desk"); cargo build --release; Pop-Location
        }
